...
print(np.min(glodap['temperature']), np.max(glodap['temperature']))
print(np.min(glodap['salinity']), np.max(glodap['salinity']))
...
tem_grid = np.zeros((len(years),12,31,180,360))
count = np.zeros(np.shape(tem_grid))
...
for yr,year in enumerate(years):
    ...
    for mn,mnth in enumerate(np.arange(1,13,1)):
        ...
        if len(tmp['temperature']) == 0:
            tem_grid[yr,mn,:,:,:] = np.nan
            continue
        ...
        for i in tqdm(np.arange(len(lon)), desc='Longitudes'):
            ...
            if len(tmp2['temperature']) > 0:
                for j in np.arange(len(lat)):
                    ...
                    if len(tmp3['temperature']) > 0:
                        for k in np.arange(len(dep)):
                            #print(k,dep_bnds[k,0], dep_bnds[k,1])
                            tmp4 = tmp3.loc[ (tmp3['depth'] >= dep_bnds[k,0]) & (tmp3['depth'] < dep_bnds[k,1]) ]
                            if len(tmp4['temperature']) > 0:
                                print("Number of observations at year", year, "and month", mnth, "after location selection =", len(tmp4['oxygen']))
                                tem_grid[yr,mn,k,j,i] = np.mean(tmp4['temperature'])
                                count[yr,mn,k,j,i] = len(tmp4['temperature'])
                            else:
                                tem_grid[yr,mn,k,j,i] = np.nan
                    else:    
                        tem_grid[yr,mn,:,j,i] = np.nan
            else:
                tem_grid[yr,mn,:,:,i] = np.nan
                
print(np.nanmin(tem_grid), np.nanmax(tem_grid))


for yr,year in enumerate(years):
    for mn,mnth in enumerate(np.arange(1,13,1)):
        print("Year =", year, "Month =", mnth, np.nanmin(tem_grid[yr,mn,:,:,:]), np.nanmax(tem_grid[yr,mn,:,:,:]))


#%% save as numpy array

os.chdir('C:\\Users\\pearseb\\Dropbox\\PostDoc\\Data')
np.savez('GLODAPv2.2020_temperature_1x1degrees.npz', tem_grid)
...
os.remove('GLODAPv2.2020_temperature_1x1degrees.nc')
data = nc.Dataset('GLODAPv2.2020_temperature_1x1degrees.nc', 'w', format='NETCDF4_CLASSIC')
...
temv = data.createVariable('temperature', np.float64, ('t', 'm', 'z', 'y', 'x'))
countv = data.createVariable('count', np.float64, ('t', 'm', 'z', 'y', 'x'))

data.description = 'regridded temperature values from GLODAPv2 2020'
...
temv.standard_name = "temperature"
...
sal_grid = np.zeros((len(years),12,31,180,360))
count = np.zeros(np.shape(sal_grid))
...
for yr,year in enumerate(years):
    ...
    for mn,mnth in enumerate(np.arange(1,13,1)):
        ...
        tmp = glodap.loc[glodap['year'] == year]
        tmp = tmp.loc[tmp['month'] == mnth]
        if len(tmp['salinity']) == 0:
            sal_grid[yr,mn,:,:,:] = np.nan
            continue
        ...
        for i in tqdm(np.arange(len(lon)), desc='Longitudes'):
            #print(i,lon_bnds[i,0], lon_bnds[i,1])
            tmp2 = tmp1.loc[ (tmp1['longitude'] >= lon_bnds[i,0]) & (tmp1['longitude'] < lon_bnds[i,1]) ]
            if len(tmp2['salinity']) > 0:
                for j in np.arange(len(lat)):
                    #print(j,lat_bnds[j,0], lat_bnds[j,1])
                    tmp3 = tmp2.loc[ (tmp2['latitude'] >= lat_bnds[j,0]) & (tmp2['latitude'] < lat_bnds[j,1]) ]
                    if len(tmp3['salinity']) > 0:
                        for k in np.arange(len(dep)):
                            #print(k,dep_bnds[k,0], dep_bnds[k,1])
                            tmp4 = tmp3.loc[ (tmp3['depth'] >= dep_bnds[k,0]) & (tmp3['depth'] < dep_bnds[k,1]) ]
                            if len(tmp4['salinity']) > 0:
                                print("Number of observations at year", year, "and month", mnth, "after location selection =", len(tmp4['oxygen']))
                                sal_grid[yr,mn,k,j,i] = np.mean(tmp4['salinity'])
                                count[yr,mn,k,j,i] = len(tmp4['salinity'])
                            else:
                                sal_grid[yr,mn,k,j,i] = np.nan
                    else:    
                        sal_grid[yr,mn,:,j,i] = np.nan
            else:
                sal_grid[yr,mn,:,:,i] = np.nan
                
print(np.nanmin(sal_grid), np.nanmax(sal_grid))


for yr,year in enumerate(years):
    for mn,mnth in enumerate(np.arange(1,13,1)):
        print("Year =", year, "Month =", mnth, np.nanmin(sal_grid[yr,mn,:,:,:]), np.nanmax(sal_grid[yr,mn,:,:,:]))


#%% save as numpy array

os.chdir('C:\\Users\\pearseb\\Dropbox\\PostDoc\\Data')
np.savez('GLODAPv2.2020_salinity_1x1degrees.npz', sal_grid)
...
#os.remove('GLODAPv2.2020_salinity_1x1degrees.nc')
data = nc.Dataset('GLODAPv2.2020_salinity_1x1degrees.nc', 'w', format='NETCDF4_CLASSIC')
...
latv = data.createVariable('lat', np.float64, ('y',))
depv = data.createVariable('dep', np.float64, ('z',))
salv = data.createVariable('salinity', np.float64, ('t', 'm', 'z', 'y', 'x'))
countv = data.createVariable('count', np.float64, ('t', 'm', 'z', 'y', 'x'))

data.description = 'regridded salinity values from GLODAPv2 2020'
data.history = "Created by Pearse J. Buchanan on 11th Dec 2020"
data.source = "GLODAPv2 (2020) downloaded from https://www.nodc.noaa.gov/ocads/oceans/GLODAPv2_2020/"
